...
A newer version of kirjasto-harjoittelu repo, which went terribly wrong due to previus version being confusing to work with. <br>
...
7. Link to the library's site (if it has one)
...
By clicking the ```<button>```, it activates a function that fetches the library via ``` new Request(); ```, makes the elements for a ```<div>``` template and then appends to the library ```<div>``` in the html file.

Once done, it will organize them in a way that is the most similar to what you have put into the search bar. If there's nothing put or you spammed into the search bar, then it will put them how it is in the API.
